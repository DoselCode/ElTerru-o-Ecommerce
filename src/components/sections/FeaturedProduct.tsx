      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-10 md:gap-12 lg:gap-16 items-center">
          ...
          <div className="md:col-span-5 flex justify-center items-center relative py-6 sm:py-8">
            ...
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[340px] h-[340px] sm:w-[460px] sm:h-[460px] md:w-[540px] md:h-[540px] rounded-full border border-terruno-burgundy/10 pointer-events-none animate-pulse-ring" />
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[270px] h-[270px] sm:w-[370px] sm:h-[370px] md:w-[430px] md:h-[430px] rounded-full border border-terruno-burgundy/15 pointer-events-none" />
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[200px] h-[200px] sm:w-[280px] sm:h-[280px] md:w-[330px] md:h-[330px] rounded-full border border-terruno-burgundy/20 pointer-events-none" />
            ...
            <Reveal variant="fade" className="relative max-w-[250px] xs:max-w-[270px] sm:max-w-[310px] md:max-w-[340px] w-full animate-float z-10">
              {/* Product Image Card */}
              <div className="relative rounded-[22px] sm:rounded-[28px] overflow-hidden shadow-2xl bg-white aspect-[3/4] sm:aspect-[4/5] border border-terruno-brown/10">
                <img
                  src={product.image}
                  alt={product.name}
                  loading="lazy"
          ...
          <div className="md:col-span-7 space-y-4 sm:space-y-6 text-terruno-brown text-center md:text-left">
            <Reveal variant="slide-right" delay={0}>
              <h2 className="font-serif text-2xl sm:text-4xl md:text-5xl font-bold text-terruno-burgundy leading-tight">
                {product.name}
              </h2>
              {product.year && (
                <p className="font-serif text-lg sm:text-2xl text-terruno-burgundy/80 mt-1">
                  {product.year}
                </p>
              )}
            </Reveal>

            <Reveal as="p" variant="fade-up" delay={90} className="text-sm sm:text-base text-terruno-brown/80 leading-relaxed max-w-xl mx-auto md:mx-0">
              {product.description}
            </Reveal>
            ...
            <Reveal variant="fade-up" delay={150} className="flex items-baseline justify-center md:justify-start gap-2.5 sm:gap-3 pt-1 sm:pt-2">
              <span className="font-serif text-2xl sm:text-4xl font-bold text-terruno-burgundy">
                ${product.price.toLocaleString('es-AR')}
              </span>
              {product.originalPrice && (
                <span className="line-through text-terruno-subtle text-sm sm:text-lg">
                  ${product.originalPrice.toLocaleString('es-AR')}
                </span>
              )}
              <span className="text-terruno-olive font-bold text-xs sm:text-sm bg-terruno-olive/10 px-2 sm:px-2.5 py-0.5 rounded-full">
                {rawDiscount}
              </span>
            </Reveal>

            {/* Product Metadata List */}
            <Reveal as="ul" variant="fade-up" delay={210} className="space-y-1.5 text-xs sm:text-sm text-terruno-subtle border-t border-b border-terruno-brown/10 py-3 sm:py-4 text-left max-w-md mx-auto md:mx-0">
              {product.winery && (
                <li className="flex items-center gap-2">
                  <span className="w-1.5 h-1.5 rounded-full bg-terruno-burgundy" />
